...
const ASSEMBLY_TYPE_OPTIONS: Array<{ value: AssemblyConfigInput["type"]; label: string; description: string }> = [
...
const MODALITY_OPTIONS: Array<{ value: AssemblyConfigInput["modality"]; label: string }> = [
...
function conferenceServiceRequiresMeetingLink(service: AssemblyConferenceService): boolean {
  return (
...
function meetingLinkPlaceholder(service: AssemblyConferenceService): string {
  switch (service) {
    case "zoom":
...
function meetingLinkHelperText(service: AssemblyConferenceService): string {
  switch (service) {
    case "zoom":
...
function isVirtualConferenceConfigOk(form: AssemblyConfigInput): boolean {
...
function getAllowedModalities(type: AssemblyConfigInput["type"]) {
...
function normalizeAssemblyForm(input: AssemblyConfigInput): AssemblyConfigInput {
...
function updateLocalDateTime(value: string, part: "date" | "time", nextValue: string) {
...
function createDefaultForm(): AssemblyConfigInput {
  return {
...
function createDefaultAccessConfig(): AssemblyAccessConfigInput {
  return {
...
function buildSlideFromPoint(item: AgendaItemInput): AgendaItemInput {
...
function createSuggestedPresentation(assemblyTitle: string): AgendaItemInput[] {
  return [
    {
...
function getStatusStyle(status: AssemblySummary["status"]) {
...
function getInvitationStatusConfig(status: AssemblyInvitationRecipientSummary["status"]) {
  const map: Record<
...
function formatInvitationSentAt(iso: string) {
  try {
